...
class ARAGSystemConfig:
    ...
    use_cached_data: bool = False  # Local cache disabled; always load from GCS
...
class ARAGSystem:
    ...
    def __init__(self, config: Optional[ARAGSystemConfig] = None):
        ...
        # Trained GNN weights status (set during initialize())
        self.krag_encoder_weights_loaded: bool = False
        self.krag_encoder_weights_path: Optional[str] = None
        self.krag_encoder_weights_error: Optional[str] = None
        self.krag_encoder_weights_source: Optional[str] = None  # local|gcs
        self.krag_encoder_weights_gcs_uri: Optional[str] = None
    ...
    def _load_trained_weights(self):
        """
        Load trained weights for the GNN encoder.

        Uses strict loading: if weights cannot be loaded, initialization fails.
        """
        models_dir = Path(self.config.model_cache_dir)
        encoder_path = models_dir / "krag_encoder.pt"
        self.krag_encoder_weights_path = str(encoder_path)
        self.krag_encoder_weights_loaded = False
        self.krag_encoder_weights_error = None
        self.krag_encoder_weights_source = None
        self.krag_encoder_weights_gcs_uri = None
        ...
        if encoder_path.exists():
            ...
            try:
                ...
                self.krag_encoder_weights_loaded = True
                self.krag_encoder_weights_source = "local"
            except Exception as e:
                self.krag_encoder_weights_error = str(e)
                raise RuntimeError(f"Failed to load GNN weights from {encoder_path}: {e}") from e
        else:
            # Fallback: try loading from GCS (gs://{bucket}/{base_path}/models/krag_encoder.pt)
            gcs_rel_path = "models/krag_encoder.pt"
            gcs_uri = f"gs://{self.config.gcs_bucket}/{self.config.gcs_base_path}/{gcs_rel_path}"
            self.krag_encoder_weights_gcs_uri = gcs_uri

            try:
                from .data.adapters import get_adapter
                adapter = get_adapter(bucket_name=self.config.gcs_bucket, base_path=self.config.gcs_base_path)
                if adapter.exists(gcs_rel_path):
                    print(f"      Downloading trained weights from {gcs_uri}...", flush=True)
                    payload = adapter.load_bytes(gcs_rel_path)
                    state = torch.load(io.BytesIO(payload), map_location="cpu")
                    self.krag_encoder.load_state_dict(state)
                    self.krag_encoder_weights_loaded = True
                    self.krag_encoder_weights_source = "gcs"
                    print("      Weights loaded successfully (GCS)", flush=True)

                    # Cache locally for next run
                    try:
                        models_dir.mkdir(parents=True, exist_ok=True)
                        encoder_path.write_bytes(payload)
                    except Exception as e:
                        print(f"      Warning: Failed to cache weights locally: {e}", flush=True)
                else:
                    self.krag_encoder_weights_error = "weights_file_missing"
                    raise FileNotFoundError(
                        f"GNN weights missing. Expected local {encoder_path} or remote {gcs_uri}."
                    )
            except Exception as e:
                self.krag_encoder_weights_error = str(e)
                raise RuntimeError(f"Failed to load GNN weights from GCS ({gcs_uri}): {e}") from e
    ...
    def load_and_index_data(self, force_reload: bool = False) -> Dict[str, Any]:
        ...
        try:
            if force_reload:
                raise ValueError(
                    "force_reload is not supported in strict mode. "
                    "Data must be loaded deterministically from GCS artifacts."
                )

            if self.config.use_cached_data:
                raise ValueError(
                    "use_cached_data is not supported in strict mode. "
                    "Local processed-data cache is disabled."
                )
            ...
            # Load embeddings from GCS (single source of truth)
            ...
            if force_reload:
                raise ValueError(
                    "force_reload is not supported for embeddings in strict mode. "
                    "Embeddings must be loaded from GCS precomputed artifacts."
                )

            if not self.data_processor.adapter.exists(DatasetPath.EMBEDDINGS):
                raise FileNotFoundError(
                    f"Missing required embeddings artifact in GCS: "
                    f"gs://{self.config.gcs_bucket}/{self.config.gcs_base_path}/{DatasetPath.EMBEDDINGS.value}"
                )
            ...
            print(
                f"  Loaded from gs://{self.config.gcs_bucket}/{self.config.gcs_base_path}/{DatasetPath.EMBEDDINGS.value}",
                flush=True
            )
            print(f"  Semantic: {semantic_embeddings.shape}, Emotion: {emotion_embeddings.shape}", flush=True)

            # Load node embeddings from GCS (single source of truth)
            node_embeddings = None
            
            if force_reload:
                raise ValueError(
                    "force_reload is not supported for node embeddings in strict mode. "
                    "Node embeddings must be loaded from GCS precomputed artifacts."
                )

            if not self.data_processor.adapter.exists(DatasetPath.NODE_EMBEDDINGS):
                raise FileNotFoundError(
                    f"Missing required node embeddings artifact in GCS: "
                    f"gs://{self.config.gcs_bucket}/{self.config.gcs_base_path}/{DatasetPath.NODE_EMBEDDINGS.value}"
                )
    ...
    def train_gnn(
        ...
        Args:
            ...
            use_multi_k: Use multi-k contrastive training for structural sensitivity
            contrastive_weight: Weight for contrastive loss term (if use_multi_k=True)
        ...
        if use_multi_k:
            print("\nUsing multi-k contrastive training for structural sensitivity...")
            train_dataset, val_dataset = trainer.prepare_multi_k_training_data(
                content_items=self.content_items,
                node_embeddings=node_embeddings,
                emotion_ground_truth=emotion_ground_truth
            )
            results = trainer.train_multi_k(train_dataset, val_dataset)
        else:
