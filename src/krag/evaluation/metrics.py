...
def compute_faithfulness_necessity_score(
    ...
    FNS = max(0, S_orig - S_pert)

    KRAG combined_score has range [-(1-alpha), alpha] with max span of exactly 1.0
    for any alpha. The absolute drop is therefore directly interpretable as a [0, 1]
    necessity score without normalization. The ratio formula (S_orig - S_pert) / S_orig
    does not apply because S_orig can be near zero while S_pert goes negative, causing
    the ratio to blow up.
    ...
    Returns:
        FNS in [0, 1] (higher = edge was more causally necessary)
    """
    if original_score <= 0:
        return 0.0

    return float(max(0.0, original_score - perturbed_score))
