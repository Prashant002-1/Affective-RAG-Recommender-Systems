...
class KRAGRetriever(BaseRetriever):
    """
    Affective-RAG retriever implementing the paper formula:
    Score(m) = α·Cosine(v_query, v_m) - (1-α)·||e_target - e_m||_2

    Combines semantic similarity with affective fidelity using Euclidean distance.
    """
    ...
    def retrieve(self, query_context: QueryContext, k: int = 10) -> List[RetrievalResult]:
        ...
        Score = α·[λ·Semantic + (1-λ)·Graph] - (1-α)·Affective
    ...
    def _compute_graph_similarity(self, content_id: str, query_embedding: np.ndarray) -> float:
        ...
        if len(query_embedding) != len(subgraph_emb):
            raise ValueError(
                "Graph similarity dimension mismatch: "
                f"query_embedding_dim={len(query_embedding)} vs subgraph_embedding_dim={len(subgraph_emb)}. "
                "Refusing to pad/truncate embeddings; ensure the GNN encoder and query embedder "
                "use consistent embedding_dim."
            )
